...
function applyStoreName() {
  ...
  document
    .querySelectorAll('[data-store-handle]')
    .forEach((el) => (el.textContent = '@' + storeSlug()));
...
function iconFor(cat) {
  const icons = {
    Wedding:
      '<svg viewBox="0 0 24 24" stroke="#fff" fill="none" stroke-width="1.3"><circle cx="9" cy="15" r="5"/><circle cx="15" cy="15" r="5"/><path d="M12 4v4"/></svg>',
    Festive:
      '<svg viewBox="0 0 24 24" stroke="#fff" fill="none" stroke-width="1.3"><path d="M4 18c0-4 3-6 8-6s8 2 8 6"/><path d="M12 12V4"/><path d="M9 4c0 2 1.5 3 3 3s3-1 3-3"/></svg>',
    'Baby Shower':
      '<svg viewBox="0 0 24 24" stroke="#fff" fill="none" stroke-width="1.3"><circle cx="12" cy="9" r="5"/><path d="M9 14l-2 6M15 14l2 6M7 9a5 5 0 0 1 10 0"/></svg>',
  };
...
function productDesigns(p) {
  return Array.isArray(p.designs) && p.designs.length
    ? p.designs
    : [{ name: p.name, image: '', images: [] }];
}
...
function cardCarouselSlides(p) {
  ...
  const seen = new Set();
  const slides = [];
  if (p.coverImage && !seen.has(p.coverImage)) {
    seen.add(p.coverImage);
    slides.push(p.coverImage);
  }
...
function createDragCarousel({
  ...
  const settleNext = () => {
    if (index < count - 1) animateTo(width * 2, index + 1);
    else snapBack();
  };
  const settlePrev = () => {
    if (index > 0) animateTo(0, index - 1);
    else snapBack();
  };
...
function wirePhotoSubGallery(scopeEl) {
  if (qvGalleryKeyHandler) {
    document.removeEventListener('keydown', qvGalleryKeyHandler);
    qvGalleryKeyHandler = null;
  }
  if (!scopeEl) return;
...
function applyDesignMetaUI() {
  ...
  document.getElementById('whatsappEnquire').href =
...
function imgViewerSlideMarkup(p, i) {
  ...
  if (!src)
    return `<div class="swatch ${p.swatch}"><div class="sw-icon">${iconFor(p.category)}</div></div>`;
...
function openImgViewer(startIndex) {
  ...
  if (imgViewerCarousel) {
    imgViewerCarousel.destroy();
    imgViewerCarousel = null;
  }
  gallery.innerHTML = '';
...
function openQuickView(id, designIndex = 0) {
  ...
  qvModalKeyHandler = (e) => {
    ...
    else if (designs.length > 1 && e.key === 'ArrowRight')
      qvDesignCarousel && qvDesignCarousel.next();
    else if (designs.length > 1 && e.key === 'ArrowLeft')
      qvDesignCarousel && qvDesignCarousel.prev();
  };
...
function closeQuickView() {
  ...
  if (qvModalKeyHandler) {
    document.removeEventListener('keydown', qvModalKeyHandler);
    qvModalKeyHandler = null;
  }
  if (qvGalleryKeyHandler) {
    document.removeEventListener('keydown', qvGalleryKeyHandler);
    qvGalleryKeyHandler = null;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  shopSearchClose.addEventListener('click', (e) => {
    e.stopPropagation();
    closeShopSearch();
  });
  document.addEventListener('click', (e) => {
    if (shopSearchWrap.classList.contains('open') && !shopSearchWrap.contains(e.target))
      closeShopSearch();
  });
  ...
  document.addEventListener('click', (e) => {
    if (sortSelectWrap.classList.contains('open') && !sortSelectWrap.contains(e.target))
      closeSort(false);
  });
